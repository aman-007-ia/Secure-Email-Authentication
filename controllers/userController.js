        ...
        if(password && password_confirmation && email){
            const user = await UserModel.findOne({email:email})
                ...
                if(password !== password_confirmation){
                    ...
                    const user = UserModel.findById(req.user._id,{email:email})
                    await UserModel.findOneAndUpdate(user,{$set:{password:newHashPassword}})
                    res.send({"status":"success","message":"Password Changed Succesfully"})
                }}
                else{
                    res.send({"status":"failed","message":"Email is not matched"})
                 }
            }
        }
        else{
        ...
        if(email){
            ...
            if(user){
                ...
                let info = await transporter.sendMail({
                    from: `amanyadav794178@gmail.com`,
                    to: user.email,
